...
static public CloudQueue CreateQueue(string storageAccountName, string storageAccountKey, string endPointAddress)
{
    ...
    CloudQueueClient queueClient = sourceStorageAccount.CreateCloudQueueClient();
...
public static async Task<IAsset> CreateAssetFromBlobDeleteOriginal(CloudBlockBlob blob, string assetName, TraceWriter log){
        IAsset newAsset = null;
        ...
        try{
            ...
            log.Info($"Asset Copied : {newAsset.Id}");
            blob.Delete();
            log.Info($"Deleted original blob");
        }
        catch(Exception ex){
            log.Info("Copy Failed");
            log.Info($"ERROR : {ex.Message}");
