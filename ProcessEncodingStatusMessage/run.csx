    ...
    if (encodingJobMsg.EventType == "JobStateChange")
    {
        ...
        if (jobState == JobState.Finished) {
            ...
            {      
                ...
                log.Error($"Unable to generate a valid publish URL. Make sure you have at least one Streaming Endpoint in Running state.");
            }
        }
        else if (jobState == JobState.Error)
        {
            log.Error($"Job {jobId} failed with an error.");
        }
    }
}